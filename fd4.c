...
void quickSort_v1(List* list, int L, int R) {
    int left = L, right = R;
    int pivot = list->arr[(L + R) / 2];

    int tmp;
    do {
        while (list->arr[left] < pivot) left++;
        while (list->arr[right] > pivot) right--;
        if (left <= right) {
            tmp = list->arr[left];
    ...
    if (L < right) quickSort_v1(list, L, right);
    if (left < R) quickSort_v1(list, left, R);
}

void quickSort_v2(List* list, int L, int R) {
    int left = L, right = R;
    int pivot = list->arr[(L + R) / 2];
    ...
    if (L < right) quickSort_v2(list, L, right);
    if (left < R) quickSort_v2(list, left, R);
}

void sortList(List* list,int type) {
    int len = getNfromList(list);
    if (type == 0) {
        quickSort_v1(list, 0, len-1);
    }
    else {
        quickSort_v2(list, 0, len-1);
    }
}
